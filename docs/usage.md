...
path, and a queue of selected phases for `all`.
...
reviews `git status --short` and the phase diff, stages only selected-phase
files, and creates one professional Conventional Commit.
...
records completed task IDs, selected-phase modified and added/created files,
focused validation commands, issues, and a styled Phase Flow Mermaid diagram
unless the user explicitly asks to omit diagrams.
